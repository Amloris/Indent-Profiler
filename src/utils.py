...
def LoadData(fname):
    ...
    if (file_ext == '.txt'):
        ...
        return 0                                 #Terminate function call
    
    #Load Header
    header_length = 4
    header_index = np.arange(0, header_length)    #The rows which contain the header
    with open(fname,'r') as fin:
        for i, line in enumerate(fin):
            if i in header_index:
                if i == 0:
                    wire_profile_val = line.strip().split(delim)[-1]
                if i == 1:
                    x_location_val = float(line.strip().split(delim)[-1])
                if i == 2:
                    total_sample_length_val = float(line.strip().split(delim)[-1])
                if i == 3:
                    pts_per_rev_val = int(line.strip().split(delim)[-1]) 
                    
                
    scan_info = ScanInfo(wire_profile_val, x_location_val,         \
                         total_sample_length_val, pts_per_rev_val)
...
class ScanInfo():
    def __init__(self, wire_profile, x_location, total_sample_length, pts_per_rev):
